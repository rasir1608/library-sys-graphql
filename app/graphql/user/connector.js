...
class UserConnector {
  ...
  queryUserRecoders({ user, recoder, book }) {
    const users = this.ctx.app.model.query(`
    SELECT 
    ${[...formatKeys('users', userMap),
    ...formatKeys('recoders', recoderMap),
    ...formatKeys('books', bookMap)]
    .join(',')}
    FROM users
    LEFT JOIN recoders
    ...
    ${[...formatCondition(user, userMap),
    ...formatCondition(recoder, recoderMap),
    ...formatCondition(book, bookMap)]
    .join(' AND ')}
    `
    : ''}
    `,
